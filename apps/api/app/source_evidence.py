...
def derive_references(quote, document):
    ...
    def attach(obj, fields, candidates):
        for field in fields:
            ...
            for page, text in candidates:
                ...
                if any(re.match(r"\s*" + re.escape(alias) + r"\s*[:|,]", text, re.I) for alias in aliases):
    ...
    consumed = {}
    all_skus = {line.supplier_sku for line in quote.line_items if line.supplier_sku}
    for line in quote.line_items:
        ...
        if indices:
            ...
            if re.match(r"\s*(supplier_sku|sku|part number)\s*[:|,]", first[1], re.I):
                end = start + 1
                while end < len(source_rows):
                    row = source_rows[end][1]
                    if (
                        "LINE ITEM" in row
                        or re.match(r"Quoted item|Subtotal|Grand total|Shipping|Tax", row, re.I)
                        or (
                            re.match(r"\s*(supplier_sku|sku)\s*[:|,]", row, re.I)
                            and any(supports("supplier_sku", sku, row) for sku in all_skus)
                        )
                    ):
                        break
                    end += 1
